...
make cli ARGS="-p methods/guest/target/riscv32im-risc0-zkvm-elf/docker/test_spel_e2e.bin \\
  <command> --arg1 value1 --arg2 value2"

# Dry run (no submission):
make cli ARGS="--dry-run -p methods/guest/target/riscv32im-risc0-zkvm-elf/docker/test_spel_e2e.bin \\
  <command> --arg1 value1"
```
...
```
test-spel-e2e/
├── test_spel_e2e_core/    # Shared types (used by guest + host)
│   └── src/lib.rs
├── methods/
│   └── guest/            # RISC Zero guest program (runs on-chain)
│       └── src/bin/test_spel_e2e.rs
├── examples/             # CLI tools
│   └── src/bin/
│       ├── generate_idl.rs    # One-liner IDL generator
│       └── test_spel_e2e_cli.rs # Three-line CLI wrapper
├── Makefile
└── test-spel-e2e-idl.json       # Auto-generated IDL
```
